...
export default function Topseries() {
  ...
  useEffect(()=>{
    axios.get(`https://api.themoviedb.org/3/tv/top_rated?api_key=6cdf1bc39c14d06f4b51e70d622928f4&language=en-US&page=1`).then (response=>{
...
const singleview=(id)=>{
  ...
  return (
    ...
    <div>
                   <h1 className='scrollh1'>Top-rated Series</h1> 
    ...
    <div className="scroll">
      ...
      {Topseries.map((tops)=>(
       ...
       <div className='imagesscrolling'>
        <img className='scrollimg' onClick={()=>{handleOpen();singleview(tops.id)}} src={`https://image.tmdb.org/t/p/original${tops.poster_path}`} alt=''/>
         </div>
         </>
        ))}
        </div>
