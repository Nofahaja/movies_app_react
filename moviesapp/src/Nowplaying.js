...
export default function Nowplaying() {
  ...
  useEffect(()=>{
    axios.get(`https://api.themoviedb.org/3/movie/now_playing?api_key=6cdf1bc39c14d06f4b51e70d622928f4&language=en-US&page=1`).then (response=>{
...
console.log(state);
  ...
  return (
    <div>
                   <h1 className='scrollh1'>Now Playing Movies</h1> 
    ...
    <div className="scroll">
      ...
      {Playing.map((play)=>(
       ...
       <div className='imagesscrolling'>
        <img className='scrollimg'onClick={()=>{handleOpen();singleview(play.id)}} src={`https://image.tmdb.org/t/p/original${play.poster_path}`} alt=''/>
         </div>
         </>
        ))}
        </div>
