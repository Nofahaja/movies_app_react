...
export default function Airingtoday() {
  ...
  const handleClose = () => setOpen(false);
    ...
    console.log(state);
  useEffect(()=>{
    axios.get(`https://api.themoviedb.org/3/tv/airing_today?api_key=6cdf1bc39c14d06f4b51e70d622928f4&language=en-US&page=1`).then (response=>{
...
const singleview=(id)=>{
  ...
  return (
    <div>
                   <h1 className='scrollh1'>Airingtoday Series</h1> 
    ...
    <div className="scroll">
      ...
      {Airingtoday.map((air)=>(
       ...
       <div className='imagesscrolling'>
        <img className='scrollimg' onClick={()=>{handleOpen();singleview(air.id)}}src={`https://image.tmdb.org/t/p/original${air.poster_path}`} alt=''/>
         </div>
         </>
        ))}
        </div>
